...
func newAccessReviewNamespace(t *transport) *AccessReviewNamespace {
	n := &AccessReviewNamespace{t: t}
	n.Dismissals = newAccessReviewDismissalsNamespace(t)
	return n
}

// AccessReviewExportParams holds the parameters for
// `client.accessReview.export`.
//
// Every field is optional; pass nil to take the defaults.
...
//
// The same review as a downloadable file, one row per finding. `format=csv` (the
// default) returns RFC 4180 CSV with every cell quoted and spreadsheet formulas
// neutralised; `format=json` returns the full response body pretty-printed.
//
// Dismissed findings are included and labelled in both formats, with the note
// and the person who accepted them: an evidence pack answers what you found
// *and* what you decided. Exports are recorded in the audit log.
//
...
//
// Raises on 400: Bad request
func (n *AccessReviewNamespace) Export(ctx context.Context, params *AccessReviewExportParams, opts ...RequestOption) (*AccessReviewResponse, error) {
	r := newRequest(http.MethodGet, "/api/org/{orgId}/access-review/export")
	if params != nil {
		r.setPath("orgId", params.OrgID)
		r.addQuery("format", params.Format)
		r.addQuery("staleDays", params.StaleDays)
	}
	var out *AccessReviewResponse
	if err := n.t.do(ctx, r, &out, opts); err != nil {
		return out, err
	}
	return out, nil
...
// binding and long-lived API key your connected accounts have synced, with the
// findings that have evidence against them: unused beyond the staleness window,
// holding administrative or wildcard permissions, past the rotation budget their
...
// excluded from `findings`, `counts`, `byRule` and the security alerts.
//
// _Requires permission: `resources:read`._
//
...
//
// Raises on 400: Bad request
func (n *AccessReviewNamespace) Get(ctx context.Context, params *AccessReviewGetParams, opts ...RequestOption) (*AccessReviewResponse, error) {
	r := newRequest(http.MethodGet, "/api/org/{orgId}/access-review")
	if params != nil {
		r.setPath("orgId", params.OrgID)
		r.addQuery("staleDays", params.StaleDays)
	}
...
func newAccessReviewDismissalsNamespace(t *transport) *AccessReviewDismissalsNamespace {
	n := &AccessReviewDismissalsNamespace{t: t}
	return n
}

// AccessReviewDismissalsCreateParams holds the parameters for
// `client.accessReview.dismissals.create`.
//
// Every field is optional; pass nil to take the defaults.
...
// The principal itself stays in `principals` either way. Idempotent: dismissing
...
//
// Raises on 400: Bad request
func (n *AccessReviewDismissalsNamespace) Create(ctx context.Context, params *AccessReviewDismissalsCreateParams, opts ...RequestOption) (*AccessReviewDismissal, error) {
	r := newRequest(http.MethodPost, "/api/org/{orgId}/access-review/dismissals")
	if params != nil {
		r.setPath("orgId", params.OrgID)
		r.setJSONBody(params.Body)
	}
	var out *AccessReviewDismissal
	if err := n.t.do(ctx, r, &out, opts); err != nil {
		return out, err
	}
	return out, nil
...
// AccessReviewDismissalsDeleteParams holds the parameters for
// `client.accessReview.dismissals.delete`.
...
// path segments because resource ids are provider-native and routinely contain
// slashes.
//
// _Requires permission: `resources:write`._
//
...
//
// Raises on 404: Not found
func (n *AccessReviewDismissalsNamespace) Delete(ctx context.Context, params AccessReviewDismissalsDeleteParams, opts ...RequestOption) error {
	r := newRequest(http.MethodDelete, "/api/org/{orgId}/access-review/dismissals")
	r.setPath("orgId", params.OrgID)
...
func newBackupsNamespace(t *transport) *BackupsNamespace {
	...
	n.Drills = newBackupsDrillsNamespace(t)
...
func newBackupsDrillsNamespace(t *transport) *BackupsDrillsNamespace {
	n := &BackupsDrillsNamespace{t: t}
	return n
}

// BackupsDrillsCreateParams holds the parameters for
// `client.backups.drills.create`.
//
// Every field is optional; pass nil to take the defaults.
...
// reporting what you did, and the person who spent Saturday restoring a database
// is rarely the person who set the recovery objective.
//
// POST /api/org/{orgId}/backups/drills
//
// Raises on 400: Bad request
func (n *BackupsDrillsNamespace) Create(ctx context.Context, params *BackupsDrillsCreateParams, opts ...RequestOption) (*RestoreDrill, error) {
	r := newRequest(http.MethodPost, "/api/org/{orgId}/backups/drills")
	if params != nil {
		r.setPath("orgId", params.OrgID)
		r.setJSONBody(params.Body)
	}
...
// BackupsDrillsDeleteParams holds the parameters for
// `client.backups.drills.delete`.
...
//
// Raises on 404: Not found
func (n *BackupsDrillsNamespace) Delete(ctx context.Context, params BackupsDrillsDeleteParams, opts ...RequestOption) error {
	r := newRequest(http.MethodDelete, "/api/org/{orgId}/backups/drills/{drillId}")
	r.setPath("orgId", params.OrgID)
	r.setPath("drillId", params.DrillID)
	return n.t.do(ctx, r, nil, opts)
}
...
// restoring a customer's database unattended costs real money, can collide with
// production, and cannot be generically verified. What the product can do is
// make the exercise scheduled, recorded and visible when it lapses.
//
// GET /api/org/{orgId}/backups/drills
//
// Raises on 400: Bad request
func (n *BackupsDrillsNamespace) Get(ctx context.Context, params *BackupsDrillsGetParams, opts ...RequestOption) (*DrillCoverageResponse, error) {
	r := newRequest(http.MethodGet, "/api/org/{orgId}/backups/drills")
	if params != nil {
		r.setPath("orgId", params.OrgID)
		r.addQuery("validDays", params.ValidDays)
	}
	var out *DrillCoverageResponse
	if err := n.t.do(ctx, r, &out, opts); err != nil {
		return out, err
	}
	return out, nil
...
func (n *BackupsDrillsNamespace) Log(ctx context.Context, params *BackupsDrillsLogParams, opts ...RequestOption) (*BackupsDrillsLogResponse, error) {
	r := newRequest(http.MethodGet, "/api/org/{orgId}/backups/drills/log")
	if params != nil {
		r.setPath("orgId", params.OrgID)
		r.addQuery("resourceId", params.ResourceID)
	}
	var out *BackupsDrillsLogResponse
	if err := n.t.do(ctx, r, &out, opts); err != nil {
		return out, err
	}
	return out, nil
...
func newChangesNamespace(t *transport) *ChangesNamespace {
	...
	n.Revert = newChangesRevertNamespace(t)
	return n
}
...
func newChangesRevertNamespace(t *transport) *ChangesRevertNamespace {
	n := &ChangesRevertNamespace{t: t}
	return n
}

// ChangesRevertCreateParams holds the parameters for
// `client.changes.revert.create`.
...
// drops out of the patch.
//
// **This is a last-moment re-read, not an atomic compare-and-swap.** The gap
// between reading a field and writing it is one provider round-trip wide, and a
// third party writing inside that gap will be overwritten without warning. It
// cannot be closed generically: the plugin update contract carries no expected
// value, ETag or version token, so no conditional write can be expressed for a
// provider that supports one. Treat the conflict detection as a strong guard
// against stale plans, not as a mutual-exclusion guarantee against other
// writers.
//
// Reverts of the *same event* are mutually exclusive: the event is claimed with
// a conditional update under a five-minute lease, so two concurrent reverts
// cannot both reach the provider and the loser gets `409`. A provider failure
// releases the claim immediately; a process that dies mid-write leaves a claim
// that expires, so an interrupted revert is retryable rather than permanently
// stuck. `revertedAt` is only set once the provider accepted the write.
//
...
// empty `appliedFields`. A resource put back by hand is not mistaken for this:
// reconciliation only happens on an event whose claim was still outstanding,
// which is the only state in which an unrecorded write is possible.
//
...
// no transaction spans a third-party cloud API and Infrawrench's database, so if
// the audit insert itself fails the response carries `auditRecorded: false` and
// the details go to the server log rather than being silently dropped.
//
// The stored resource snapshot is deliberately left untouched, so the next poll
// observes the reverted state and records it as an ordinary change event.
//
...
// Raises on 404: Not found
//
// Raises on 409: Already reverted, another revert holds the event, nothing in
// the plan is writable, or this attempt was superseded mid-write (its lease
// lapsed). The body carries `code: change_revert_conflict` for all but the
// writability case, and `appliedFields` when the provider write had already
// landed.
//
...
//
// Raises on 500: The provider accepted the write but it could not be recorded
// against the event. The resource *has* been put back; `appliedFields` names
// what changed. A later retry reconciles the timeline.
//
// Raises on 502: The provider couldn't be read. Nothing was written.
func (n *ChangesRevertNamespace) Create(ctx context.Context, params ChangesRevertCreateParams, opts ...RequestOption) (*RevertApplyResponse, error) {
	r := newRequest(http.MethodPost, "/api/org/{orgId}/changes/{changeId}/revert")
	r.setPath("orgId", params.OrgID)
	r.setPath("changeId", params.ChangeID)
	var out *RevertApplyResponse
	if err := n.t.do(ctx, r, &out, opts); err != nil {
		return out, err
	}
	return out, nil
...
// writable at all is the plugin's own edit-form rule (`editable`, minus `secret`
// and `association` kinds), so a revert can never issue a provider call an edit
// could not.
//
// Gated on `resources:write` rather than `resources:read`: the plan names the
// write it is offering to make.
//
// GET /api/org/{orgId}/changes/{changeId}/revert
//
// Raises on 404: Not found
//
// Raises on 502: The provider couldn't be read, so no plan can be made safely.
// Nothing was written.
func (n *ChangesRevertNamespace) Get(ctx context.Context, params ChangesRevertGetParams, opts ...RequestOption) (*RevertPreviewResponse, error) {
	r := newRequest(http.MethodGet, "/api/org/{orgId}/changes/{changeId}/revert")
	r.setPath("orgId", params.OrgID)
	r.setPath("changeId", params.ChangeID)
